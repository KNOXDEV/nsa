/* reactions is an append-only event log (RESUME redelivery and add/remove cycles leave duplicate
   and contradictory rows). This view is the current state -- latest event per (message, user,
   emoji), adds only. Query this, not raw reactions, for "who is currently reacting". */
